    ...
    public static function get_option_name($value,$key=NULL){
        ...
        if(count($arr_val)>1){
            ...
            if($arr_val[0] AND $arr_val[0]!="i"){
                $value_txt.=" ";
                $value_txt.=sprintf(__("from: %s",'woocommerce-products-filter'),date($format,$arr_val[0]));
            }
            if($arr_val[1] AND $arr_val[1]!="i"){
                $value_txt.=" ";
                $value_txt.=sprintf(__("to: %s",'woocommerce-products-filter'),date($format,$arr_val[1]));
            }            
