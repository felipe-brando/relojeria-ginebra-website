...
if(!empty($meta_options)){
 ...
 {
     $woof_value =  explode( ",",$request['mselect_'.$meta_key]);
 }
...
<div data-css-class="woof_meta_mselect_container" class="woof_meta_mselect_container woof_container woof_container_<?php echo $meta_key ?> woof_container woof_container_<?php echo $meta_key ?>  woof_container_<?php echo "mselect_".$meta_key ?>">
        ...
        <div class="woof_container_inner woof_container_inner_meta_mselect">
            ...
            <div class="<?php echo $css_classes ?>">
                <select class="woof_meta_mselect woof_meta_mselect_<?php echo $meta_key ?>" name="<?php echo "mselect_".$meta_key ?>"multiple="multiple" data-placeholder="<?php echo WOOF_HELPER::wpml_translate(null,$options['title']) ?>" >
                     ...
                     <?php foreach ($meta_options as $key => $option) : ?>
                         ...
                         if (!in_array($key+1,$woof_value))
                         {
                             if ($show_count)
                             {
                                 ...
                                 {
                                     $count_data=array();
                                     $count = $WOOF->dynamic_count(array(), 'mselect', (isset($_REQUEST['additional_taxes']))?$_REQUEST['additional_taxes']: "",$meta_field);
                                      $count_string = '(' . $count . ')';
                                 } else
                                 {
                                     $count=1;
                         ...
                         ?>
                         <option <?php if ($show_count AND $count == 0 AND !in_array($key+1,$woof_value)): ?>disabled=""<?php endif; ?> value="<?php echo $key+1 ?>" <?php echo selected(in_array($key+1,$woof_value)) ?>>
                             <?php
...
foreach ($woof_value as $key_val) {
             ...
             ?>   
             <input type="hidden" value="<?php echo WOOF_HELPER::wpml_translate(null,$curr_title );?>" data-anchor="woof_n_<?php echo "mselect_".$meta_key ?>_<?php echo $key_val ?>" />
<?php } ?>
