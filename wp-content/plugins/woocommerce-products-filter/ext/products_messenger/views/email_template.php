...
<div class="woof_text_email" style="color: #4a4a4e; margin-bottom: 30px;" ><?php echo $text_email ?></div>
...
<?php if ($last_email) { ?>
    <div class="last_email"><?php esc_html_e('Attention! This is the last email. If you want to continue get such emails -> Go by next link and subscribe again', 'woocommerce-products-filter') ?>
        - <a href="<?php echo $subscr['link'] . "&orderby=date&order=DESC" ?>"><?php esc_html_e('Subscribe ', 'woocommerce-products-filter'); ?></a>
    </div>
<?php } ?>
<div class="woof_subscr"><p><?php esc_html_e('If you want to Unsubscribe from this newsletter', 'woocommerce-products-filter') ?> - <a href="<?php echo $unsobscr_link ?>"><?php esc_html_e('unsubscribe', 'woocommerce-products-filter') ?></a> </p></div>
    ...
    if ($products->have_posts()) {
        ...
        if ($i < count($products->posts)) {
            ...
            <div style="margin-top: 20px" class="woof_more_text" >
                <p style="text-align: center;font-size: 18px;" ><a style="padding: 4px;border: 2px solid #557DA1; text-decoration: none;" href="<?php echo $subscr['link'] . "&orderby=date&order=DESC" ?>"><?php esc_html_e('See more new products...', 'woocommerce-products-filter') ?></a></p>
            </div> 
            <?php
        }
    } else {
        echo esc_html__('No products found', 'woocommerce-products-filter');
    }
    wp_reset_postdata();
    ?>
</ul><!--/.products-->
