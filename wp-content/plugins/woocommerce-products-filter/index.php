...
final class WOOF {
    ...
    public function print_plugin_options() {
        ...
        var woof_ext_path = "<?php echo $this->get_custom_ext_path() ?>";
        var woof_ext_url = "<?php echo $add_ext_url ?>";
        var woof_ext_custom = "<?php echo $is_custom_extensions ?>";
        var woof_is_free_ver = "<?php echo $this->is_free_ver ?>";
        <?php
        $stxt = ob_get_clean();
    ...
    public function wp_load_js() {
        ...
        <?php endif; ?>
        var woof_m_b_container ="<?php echo apply_filters('woof_mobile_btn_place_container', '.woocommerce-products-header'); ?>";  
		...
		?>	
			woof_really_curr_tax = {term_id:<?php echo intval($curr_tax->term_id) ?>, taxonomy: "<?php echo $curr_tax->taxonomy ?>"};
		<?php				
		}	
        ?>
        ...
        var woof_toggle_closed_image = "<?php esc_html_e((isset($this->settings['toggle_closed_image']) AND!empty($this->settings['toggle_closed_image'])) ? $this->settings['toggle_closed_image'] : WOOF_LINK . 'img/plus3.png') ?>";
        var woof_toggle_opened_image = "<?php esc_html_e((isset($this->settings['toggle_opened_image']) AND!empty($this->settings['toggle_opened_image'])) ? $this->settings['toggle_opened_image'] : WOOF_LINK . 'img/minus3.png') ?>";
                ...
                if (isset($taxonomies[$key])) {
                    if (isset($this->settings['tax'][$key]) && $this->settings['tax'][$key] != 0) {
                        ...
                        if (isset($this->settings["custom_tax_label"][$key]) AND $this->settings["custom_tax_label"][$key]) {
                            ...
                            if ($this->settings["tax_type"][$key] == 'select_hierarchy') {
    ...
    public function woocommerce_before_shop_loop() {
        ...
        //the $_REQUEST['woof_before_shop_loop_done'] - is just key lock here
