...
//Defaults
        ...
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title', 'woocommerce-products-filter') ?>:</label>
            <input class="widefat" type="text" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" />
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('additional_text_before'); ?>"><?php esc_html_e('Additional text before', 'woocommerce-products-filter') ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('redirect'); ?>"><?php esc_html_e('Redirect to', 'woocommerce-products-filter') ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('woof_start_filtering_btn'); ?>"><?php esc_html_e('Hide search form by default and show one button instead', 'woocommerce-products-filter') ?>:</label>
            <?php
            $options = array(
        ...
        <p>
            <label for="<?php echo $this->get_field_id('dynamic_recount'); ?>"><?php esc_html_e('Dynamic recount', 'woocommerce-products-filter') ?>:</label>
            <?php
            $options = array(
        ...
        <p>
            <label for="<?php echo $this->get_field_id('autosubmit'); ?>"><?php esc_html_e('Autosubmit', 'woocommerce-products-filter') ?>:</label>
            <?php
            $options = array(
                -1 => esc_html__('Default', 'woocommerce-products-filter'),
                0 => esc_html__('No', 'woocommerce-products-filter'),
        ...
        <p>
            <label for="<?php echo $this->get_field_id('btn_position'); ?>"><?php esc_html_e('Submit button position', 'woocommerce-products-filter') ?>:</label>
            <?php
            $options = array(
        ...
        <p>
            <label for="<?php echo $this->get_field_id('ajax_redraw'); ?>"><?php esc_html_e('Form AJAX redrawing', 'woocommerce-products-filter') ?>:</label>
            <?php
            $options = array(
                0 => esc_html__('No', 'woocommerce-products-filter'),
                1 => esc_html__('Yes', 'woocommerce-products-filter')
        ...
        <p>
            <label for="<?php echo $this->get_field_id('mobile_mode'); ?>"><?php esc_html_e('Mobile mode', 'woocommerce-products-filter') ?>:</label>
            <?php
            $options = array(
                0 => esc_html__('No', 'woocommerce-products-filter'),
                1 => esc_html__('Yes', 'woocommerce-products-filter')
            ...
            ?>
            <select class="widefat" id="<?php echo $this->get_field_id('mobile_mode') ?>" name="<?php echo $this->get_field_name('mobile_mode') ?>">
                <?php foreach ($options as $k => $val) : ?>
                    <option <?php selected($instance['mobile_mode'], $k) ?> value="<?php echo $k ?>" class="level-0"><?php echo $val ?></option>
                <?php endforeach; ?>
            </select>
