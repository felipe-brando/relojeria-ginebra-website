...
final class WOOF_HELPER {
    ...
    public static function draw_title_toggle($show, $block_is_closed) {
        ...
        if ($toggle_type == 'text' OR empty($toggle_image)) {
            ?>
            <a href="javascript: void(0);" title="<?php esc_html_e('toggle', 'woocommerce-products-filter') ?>" class="woof_front_toggle woof_front_toggle_<?php echo $condition ?>" data-condition="<?php echo $condition ?>"><?php echo $toggle_text ?></a>
            <?php
        } else {
            ?>
            <a href="javascript: void(0);" title="<?php esc_html_e('toggle', 'woocommerce-products-filter') ?>" class="woof_front_toggle woof_front_toggle_<?php echo $condition ?>" data-condition="<?php echo $condition ?>">
                <img src="<?php echo $toggle_image ?>" alt="<?php esc_html_e('toggle', 'woocommerce-products-filter') ?>" />
            </a>
            <?php
        }
    }
