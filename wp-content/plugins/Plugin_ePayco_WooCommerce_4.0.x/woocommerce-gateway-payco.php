...
//update_post_meta($post->ID, 'title', $title);
              var_dump($ref_payco, $order_id,"NOT");
                                  //Validamos la firma
                    if ($order_id!="" && $ref_payco!="") {
                        $order = new WC_Order($order_id);
                        $signature = hash('sha256',
...
// var_dump($signature, trim($validationData['x_signature']));
// die();
 ...
 if($current_state=="epayco-failed" || $current_state=="failed"){
                    ...
                    if($current_state == 'on-hold' && ((int)$validationData['x_cod_transaction_state'] == 2 || (int)$validationData['x_cod_transaction_state'] == 4) && EpaycoOrder::ifStockDiscount($order_id)){
