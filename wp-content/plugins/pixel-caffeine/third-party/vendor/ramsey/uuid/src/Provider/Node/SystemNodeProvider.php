 ...
 */
namespace PixelCaffeine\Dependencies\Ramsey\Uuid\Provider\Node;
 ...
 */
class SystemNodeProvider implements \PixelCaffeine\Dependencies\Ramsey\Uuid\Provider\NodeProviderInterface
{
    /**
    ...
    protected function getSysfs()
    {
        ...
        if (\strtoupper(\constant('PHP_OS')) === 'LINUX') {
            ...
            $macs = \array_filter($macs, function ($mac) {
                return $mac !== '00:00:00:00:00:00' && \preg_match('/^([0-9a-f]{2}:){5}[0-9a-f]{2}$/i', $mac);
            });
            $mac = \reset($macs);
        }
        return $mac;
