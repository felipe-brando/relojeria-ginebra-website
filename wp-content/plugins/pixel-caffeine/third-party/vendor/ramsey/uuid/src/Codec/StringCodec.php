 ...
 */
namespace PixelCaffeine\Dependencies\Ramsey\Uuid\Codec;
...
use PixelCaffeine\Dependencies\Ramsey\Uuid\UuidInterface;
/**
 * StringCodec encodes and decodes RFC 4122 UUIDs
 *
 * @link http://tools.ietf.org/html/rfc4122
 */
class StringCodec implements \PixelCaffeine\Dependencies\Ramsey\Uuid\Codec\CodecInterface
{
    /**
     ...
     */
    public function __construct(\PixelCaffeine\Dependencies\Ramsey\Uuid\Builder\UuidBuilderInterface $builder)
    {
     ...
     */
    public function encode(\PixelCaffeine\Dependencies\Ramsey\Uuid\UuidInterface $uuid)
    {
     ...
     */
    public function encodeBinary(\PixelCaffeine\Dependencies\Ramsey\Uuid\UuidInterface $uuid)
    {
    ...
    {
        $nameParsed = \str_replace(['urn:', 'uuid:', '{', '}', '-'], '', $encodedUuid);
        ...
        // contain dashes.
        $components = [\substr($nameParsed, 0, 8), \substr($nameParsed, 8, 4), \substr($nameParsed, 12, 4), \substr($nameParsed, 16, 4), \substr($nameParsed, 20)];
        $nameParsed = \implode('-', $components);
        if (!\PixelCaffeine\Dependencies\Ramsey\Uuid\Uuid::isValid($nameParsed)) {
            throw new \PixelCaffeine\Dependencies\Ramsey\Uuid\Exception\InvalidUuidStringException('Invalid UUID string: ' . $encodedUuid);
        }
        return $components;
    }
    /**
