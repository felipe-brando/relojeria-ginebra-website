...
class LockHandler
{
    ...
    public function __construct($name, $lockPath = null)
    {
        ...
        if (!\is_dir($lockPath)) {
            $fs = new \PixelCaffeine\Dependencies\Symfony\Component\Filesystem\Filesystem();
            $fs->mkdir($lockPath);
        }
        if (!\is_writable($lockPath)) {
            throw new \PixelCaffeine\Dependencies\Symfony\Component\Filesystem\Exception\IOException(\sprintf('The directory "%s" is not writable.', $lockPath), 0, null, $lockPath);
        }
    ...
    public function lock($blocking = \false)
    {
        ...
        if (!$this->handle) {
            throw new \PixelCaffeine\Dependencies\Symfony\Component\Filesystem\Exception\IOException($error, 0, null, $this->file);
        }
