...
class QLWAPP_Contact_Controller extends QLWAPP_Controller {
  ...
  function add_js() {
    if (isset($_GET['page']) && ($_GET['page'] === 'qlwapp_contacts')) {
      $contact_model = new QLWAPP_Contact();
      wp_enqueue_media();
      $admin_contact = include_once( QLWAPP_PLUGIN_DIR. 'assets/backend/js/contact.asset.php');
      wp_enqueue_script('qlwapp-admin-contact', plugins_url('/assets/backend/js/contact.js', QLWAPP_PLUGIN_FILE), $admin_contact['dependencies'], $admin_contact['version'], true);
